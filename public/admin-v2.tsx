...
import { 
  ...
  AlertTriangle, ChevronLeft, ChevronRight, ChevronDown, Activity, X, Sliders, Wifi,
...
// Version: 2026-02-03 - Default collapsed sidebar + URL hash navigation
...
const RuaLogo = ({ className = "" }: { className?: string }) => {
  ...
  return (
    <svg viewBox="0 0 200 150" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      ...
      <path d="M10 95 H 45 L 60 65 L 80 115 L 100 55 L 115 95 H 190" stroke="currentColor" strokeWidth="14" strokeLinecap="round" strokeLinejoin="round" className="text-white" />
...
const LogsView = ({ t, setModalConfig, closeModal }: any) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap lg:flex-nowrap gap-2 lg:gap-3 items-center justify-between lg:justify-start">
        <div className="flex flex-wrap gap-2 lg:gap-3 items-center flex-1">
          <FilterDropdown 
            label="状态"
          ...
          {hasActiveFilters && (
            <button 
              ...
              className="text-white/60 hover:text-white text-xs lg:text-sm px-2 lg:px-3 py-2 hover:bg-white/10 rounded-lg transition-colors"
            >
              清除
            </button>
          )}
        ...
        <div className="flex items-center gap-2">
          {selectedLogs.size > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 text-red-400 hover:text-red-300 hover:bg-red-500/10 text-xs lg:text-sm px-2 lg:px-3 py-2 rounded-lg transition-colors"
            >
              <Trash2 className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">删除 ({selectedLogs.size})</span>
              <span className="sm:hidden">删除</span>
            </button>
          )}
          
          <div className="text-white/40 text-xs lg:text-sm whitespace-nowrap">
        ...
        <table className="w-full text-sm">
            <thead className="bg-white/5 text-white/40 text-xs font-black uppercase sticky top-0 backdrop-blur-md z-10">
              <tr>
                ...
                <th className="hidden md:table-cell p-4 text-left">{t('labels.recipient')}</th>
            ...
            <tbody className="divide-y divide-white/5">
              {paginatedLogs.map((log: MailLog) => (
                <tr key={log.id} className={`hover:bg-white/[0.02] group ${selectedLogs.has(log.id) ? 'bg-primary/10' : ''}`}>
                  ...
                  <td className="p-4 font-mono text-white/50 text-xs lg:text-sm">{new Date(log.sentAt).toLocaleString()}</td>
                  <td className="hidden md:table-cell p-4 font-bold text-xs lg:text-sm">{log.recipient}</td>
                  <td className="p-4 text-white/70 max-w-[120px] md:max-w-[150px] truncate text-xs lg:text-sm">{log.subject}</td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between gap-4 mt-4">
          <div className="text-white/40 text-xs lg:text-sm">
            显示 {(currentPage - 1) * itemsPerPage + 1}-{Math.min(currentPage * itemsPerPage, filteredLogs.length)} / 共 {filteredLogs.length} 条
          </div>
          ...
          <div className="flex items-center gap-1 lg:gap-2">
            <button
              ...
              className="p-2 rounded-lg bg-white/5 hover:bg-white/10 disabled:opacity-30 disabled:hover:bg-white/5 text-white/60 hover:text-white transition-all"
            >
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`w-8 h-8 lg:w-10 lg:h-10 rounded-lg text-xs lg:text-sm font-bold transition-all ${
                      currentPage === pageNum
          ...
          <div className="flex items-center gap-2">
            <span className="text-white/40 text-xs lg:text-sm">跳转</span>
            <input
              type="number"
              min={1}
              max={totalPages}
              ...
              className="w-14 lg:w-16 px-2 py-1 bg-white/5 border border-white/10 rounded-lg text-white text-xs lg:text-sm text-center focus:outline-none focus:ring-2 focus:ring-primary"
            />
            <span className="text-white/40 text-xs lg:text-sm">/ {totalPages}</span>
          </div>
        </div>
      )}
    </div>
...
const NavItem = ({ id, icon: Icon, label, collapsed, activeTab, setActiveTab }: any) => (
  <button 
    ...
    className={`flex items-center ${collapsed ? 'justify-center px-2' : 'gap-4 px-6'} py-4 rounded-2xl transition-all duration-300 group ${activeTab === id ? 'bg-white/10 text-primary shadow-[inset_0_0_20px_rgba(255,255,255,0.02)]' : 'text-white/40 hover:text-white hover:bg-white/5'} w-full`} 
